/**
 * International dialing codes used by the registration phone-number field.
 *
 * Each country is keyed by its ISO 3166-1 alpha-2 code (e.g. "AU"), which is
 * unique — dial codes are NOT (e.g. +1 is shared by the US, Canada and others).
 * The flag emoji is derived from the ISO code at render time, so we don't have
 * to hard-code ~200 flag characters here.
 */

export interface CountryCode {
  /** ISO 3166-1 alpha-2 code, e.g. "AU". Unique key. */
  iso: string;
  /** Human-readable country name. */
  name: string;
  /** International dial code including the leading "+", e.g. "+61". */
  dial: string;
}

/** Default country selected in the dropdown (this is "Wheels of Australia"). */
export const DEFAULT_COUNTRY_ISO = 'AU';

/**
 * Turn an ISO alpha-2 code into its flag emoji by mapping each letter to its
 * Unicode "regional indicator symbol" (🇦 = A, 🇺 = U, etc.).
 */
...
/** Quick lookup of a country by ISO code. */
const BY_ISO: Record<string, CountryCode> = COUNTRY_CODES.reduce(
  (map, c) => {
    map[c.iso] = c;
    return map;
  },
...
/** Return the dial code (e.g. "+61") for an ISO code, falling back to the default. */
export function dialCodeFor(iso: string): string {
  return (BY_ISO[iso] ?? BY_ISO[DEFAULT_COUNTRY_ISO]).dial;
}
