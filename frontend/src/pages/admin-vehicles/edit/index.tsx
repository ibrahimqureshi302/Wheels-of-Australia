...
const VehiclesEditPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    Promise.all([adminVehiclesApi.get(id), rentalsApi.list()])
      .then(([v, rentalList]) => {
        if (!active) return;
        setRentals(rentalList);
        setImages(v.imageUrls);
        setFormData({
          make: v.make,
          model: v.model,
          ...
          status: v.status,
          rentPricePerDay: v.rentPricePerDay != null ? String(v.rentPricePerDay) : '',
          minimumRentalDays: v.minimumRentalDays != null ? String(v.minimumRentalDays) : '',
          odometerKm: v.odometerKm != null ? String(v.odometerKm) : '',
          lastOilChangeDate: v.lastOilChangeDate || '',
          lastRegoPaymentDate: v.lastRegoPaymentDate || '',
          regoDueDate: v.regoDueDate || '',
          maintenanceNote: v.maintenanceNote || '',
          rentalId: v.rentalId,
        });
      })
      .catch(() => { if (active) showError('Failed to load vehicle.'); })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminVehiclesApi.update(id, {
      ...
      showError((err as { message?: string })?.message || 'Failed to update vehicle.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 }, maxWidth: { xs: '100%', sm: 560, md: 720 }, mx: 'auto' }}>
      <motion.div variants={fadeVariants} initial="initial" animate="animate">
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>Update vehicle details and ownership.</Typography>
