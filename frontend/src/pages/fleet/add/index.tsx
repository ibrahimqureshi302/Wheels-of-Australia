...
import { addFleetVehicle } from '../../../services/rental/fleet';
...
const FleetAddPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fieldsOk || !imagesOk || !maintOk) {
      showError(t('fleet.fillAllFields', 'Please fill in all fields before adding the vehicle.'));
      return;
    }
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 }, maxWidth: { xs: '100%', sm: 560, md: 720 }, mx: 'auto' }}>
      <motion.div variants={fadeVariants} initial="initial" animate="animate">
        <Typography variant="h4" gutterBottom>{t('fleet.addVehicleTitle')}</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>{t('fleet.addVehicleSubtitle')}</Typography>
        ...
        <Box component="form" onSubmit={handleSubmit}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2, mt: 3 }}>
            ...
            <Button type="submit" variant="contained" disabled={submitting}>{t('fleet.addVehicle')}</Button>
          </Box>
        </Box>
      </motion.div>
    </Box>
