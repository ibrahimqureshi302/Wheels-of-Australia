...
const DriversSkeleton: React.FC = () => (
  <motion.div key="skeleton" variants={listVariants} initial="initial" animate="animate" exit="exit">
    <Paper
      variant="outlined"
      sx={{
        borderRadius: 2.5,
    ...
    >
      <Box sx={{ display: 'flex', p: 1.5, gap: 2, borderBottom: '1px solid', borderColor: 'divider', bgcolor: 'action.hover' }}>
        {[1, 2, 3, 4].map((i) => (
          <Skeleton key={i} variant="rounded" width={i === 1 ? 120 : 80} height={28} sx={{ borderRadius: 1 }} />
        ))}
      </Box>
      {[1, 2, 3, 4, 5].map((i) => (
